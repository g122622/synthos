...
- **兴趣评分**：对聊天内容进行语义相关性评分
- **邮件报告**：将生成的报告通过邮件发送
- **Agent 变量空间（CAVM 风格）**：Agent 可通过 `var_set/var_get/var_list/var_delete` 持久化与复用中间结果，提升多轮研究与分析能力
- **深度研究（Planner + 并行分析）**：Agent 可通过 `deep_research` 将问题拆解为多个任务并并行分析，产出 CoA 块并写入变量空间
