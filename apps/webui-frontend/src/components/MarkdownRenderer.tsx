...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content, showCopyButton = true, className = "" }) => {
        ...
        <div className={`prose prose-sm max-w-none dark:prose-invert ${className}`}>
            <ReactMarkdown
                components={{
                    ...
                    pre: ({ node, ...props }) => <pre className="bg-gray-100 dark:bg-gray-800 p-4 rounded-md overflow-x-auto my-3" {...props} />,
                    a: ({ node, ...props }) => <a className="text-blue-600 hover:underline" rel="noopener noreferrer" target="_blank" {...props} />,
                    table: ({ node, ...props }) => (
