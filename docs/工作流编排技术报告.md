# Synthos 工作流编排技术报告

> 版本：v1.1
> 更新日期：2026-02-07
> 适用对象：Synthos 开发者、架构师

---

## 📖 目录

1. [技术背景与动机](#1-技术背景与动机)
2. [系统架构设计](#2-系统架构设计)
3. [核心模块详解](#3-核心模块详解)
4. [数据流与执行模型](#4-数据流与执行模型)
5. [持久化方案](#5-持久化方案)
6. [tRPC 通信链路](#6-trpc-通信链路)
7. [前端技术选型](#7-前端技术选型)
8. [性能与优化](#8-性能与优化)
9. [测试策略](#9-测试策略)
10. [扩展性设计](#10-扩展性设计)
11. [已知问题与未来规划](#11-已知问题与未来规划)

---

## 1. 技术背景与动机

### 1.1 改造前的问题

在引入工作流引擎之前，Synthos 的 `orchestrator` 模块采用 **硬编码脚本式** 的任务编排方式：

```typescript
// 旧代码（已废弃）
const provideDataSuccess = await scheduleAndWaitForJob(TaskHandlerTypes.ProvideData, {...});
if (!provideDataSuccess) throw new Error("ProvideData 失败");

const preprocessSuccess = await scheduleAndWaitForJob(TaskHandlerTypes.Preprocess, {...});
if (!preprocessSuccess) throw new Error("Preprocess 失败");

// ... 5 步串行逻辑
```

**主要问题**：
1. **缺乏灵活性**：流程固定在代码中，修改需要重新编译部署
2. **无法并行**：即使 `GenerateEmbedding` 和 `InterestScore` 可以并行，也只能串行执行
3. **无条件分支**：无法根据运行时状态动态选择路径
4. **难以复用**：每次新增流程都需要写一遍调度逻辑
5. **可观测性差**：缺少统一的执行状态管理和监控
6. **断点续跑困难**：流程失败后需要从头重跑

### 1.2 引入工作流引擎的目标

**核心目标**：
- ✅ **可视化编排**：通过拖拽节点和连线定义流程，无需编码
- ✅ **并行执行**：支持 DAG 的并行分支，提升吞吐量
- ✅ **条件分支**：根据节点输出动态决定路径
- ✅ **断点续跑**：持久化执行状态，失败后从断点恢复
- ✅ **实时监控**：tRPC subscription 推送节点状态变化
- ✅ **解耦设计**：引擎与具体任务解耦，通过适配器模式集成 Agenda

### 1.3 技术选型原则

| 决策点 | 选型结果 | 理由 |
|--------|---------|------|
| **引擎模式** | 自研 DAG 引擎 | 现有开源方案（如 Temporal、n8n）过重，自研可精确控制与 Agenda 的集成边界 |
| **前端框架** | React Flow v12 | 成熟的流程图库，支持拖拽、缩放、自定义节点，社区活跃 |
| **状态管理** | Zustand | 轻量级（3KB），React Flow 官方推荐，无 Redux 的繁琐样板代码 |
| **通信协议** | tRPC (HTTP + WebSocket) | 类型安全的 RPC 框架，支持 subscription 实时推送 |
| **持久化** | SQLite | 轻量级，无需额外服务，支持 ACID 事务 |
| **拓扑排序算法** | Kahn 算法 | 经典 DAG 拓扑排序算法，时间复杂度 O(V+E) |

---

## 2. 系统架构设计

### 2.1 整体架构图

```
┌─────────────────────────────────────────────────────────────────┐
│                         WebUI Frontend                          │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐   │
│  │ React Flow   │  │ Zustand      │  │ tRPC Client          │   │
│  │ 画布/节点组件 │  │ 工作流状态    │  │ (WebSocket)          │   │
│  └──────────────┘  └──────────────┘  └──────────────────────┘   │
└─────────────────────────────┬───────────────────────────────────┘
                              │ HTTP/WS (:3002/trpc)
┌─────────────────────────────▼───────────────────────────────────┐
│                       WebUI Backend                             │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │ tRPC Router (转发层)                                      │   │
│  │  - /trpc/listWorkflows                                   │   │
│  │  - /trpc/triggerWorkflow                                 │   │
│  │  - /trpc/onExecutionUpdate (subscription)                │   │
│  └────────────────────┬─────────────────────────────────────┘   │
└───────────────────────┼─────────────────────────────────────────┘
                        │ tRPC Client
┌───────────────────────▼─────────────────────────────────────────┐
│                      Orchestrator                               │
│  ┌────────────────────────────────────────────────────────┐     │
│  │ tRPC Server (:8081)                                    │     │
│  │  - OrchestratorRPCImpl                                 │     │
│  │  - EventEmitter → WebSocket 推送                       │     │
│  └──────────────┬─────────────────────────────────────────┘     │
│                 │                                               │
│  ┌──────────────▼─────────────────────────────────────────┐     │
│  │ WorkflowExecutor (事件驱动 DAG 引擎)                    │     │
│  │  - DagParser (拓扑排序、环检测)                         │     │
│  │  - ExecutionContext (节点输出存储)                      │     │
│  │  - NodeExecutionStrategy (重试/超时)                   │     │
│  │  - ConditionEvaluator (条件分支)                       │     │
│  └──────────────┬─────────────────────────────────────────┘     │
│                 │                                               │
│  ┌──────────────▼─────────────────────────────────────────┐     │
│  │ AgendaNodeExecutorAdapter                              │     │
│  │  - 调用 agendaInstance.now()                           │     │
│  │  - 轮询 MongoDB (Agenda jobs 集合)                      │     │
│  └──────────────┬─────────────────────────────────────────┘     │
│                 │                                               │
│  ┌──────────────▼─────────────────────────────────────────┐     │
│  │ ExecutionPersistence (SQLite)                          │     │
│  │  - 保存执行快照 (WorkflowExecution)                     │     │
│  │  - 支持断点续跑                                         │     │
│  └────────────────────────────────────────────────────────┘     │
└─────────────────────────────────────────────────────────────────┘
                        │
                        │ MongoDB
┌───────────────────────▼─────────────────────────────────────────┐
│                    Agenda Jobs                                  │
│  - ProvideData / Preprocess / AISummarize / ...                 │
│  - 分布在 data-provider, preprocessing, ai-model 进程            │
└─────────────────────────────────────────────────────────────────┘
```

### 2.2 核心设计原则

#### 2.2.1 依赖倒置（Dependency Inversion）

引擎不依赖具体任务实现，通过 `INodeExecutorAdapter` 接口解耦：

```typescript
interface INodeExecutorAdapter {
    executeTaskNode(nodeId: string, taskType: string, params: Record<string, any>, context: ExecutionContext): Promise<NodeExecutionResult>;
    executeScriptNode(nodeId: string, scriptCode: string, context: ExecutionContext): Promise<NodeExecutionResult>;
    executeHttpNode(nodeId: string, httpConfig: HttpNodeConfig, context: ExecutionContext): Promise<NodeExecutionResult>;
}
```

生产环境使用 `AgendaNodeExecutorAdapter`，测试环境可使用 `MockNodeExecutorAdapter`。

#### 2.2.2 事件驱动（Event-Driven）

`WorkflowExecutor` 继承自 `EventEmitter`，在关键节点发射事件：

- `nodeStarted(nodeId, timestamp)`
- `nodeCompleted(nodeId, result)`
- `nodeFailed(nodeId, error)`
- `workflowCompleted(executionId)`
- `workflowFailed(executionId, error)`

外部订阅者（如 tRPC Server）监听这些事件并推送给前端。

#### 2.2.3 状态持久化（Persistence）

每次节点状态变更时，立即 `upsert` 到 SQLite：

```typescript
await this._persistence.saveExecution({
    executionId,
    workflowId,
    status: "running",
    nodeStates: { [nodeId]: { status: "running", startedAt: Date.now() } },
    snapshot: structuredClone(workflowDefinition)
});
```

断点续跑时，从 SQLite 恢复快照，跳过已完成的节点。

---

## 3. 核心模块详解

### 3.1 DagParser（拓扑排序与校验）

**职责**：
- 解析 `nodes[]` + `edges[]` 构建邻接表
- **Kahn 算法** 拓扑排序，生成 `ExecutionPlan`（分层节点列表）
- **环检测**：拓扑排序后若有剩余节点，说明存在环
- **连通性校验**：所有节点从 start 可达

**关键代码**：

```typescript
// applications/orchestrator/src/core/DagParser.ts
export class DagParser {
    public parse(definition: WorkflowDefinition): ExecutionPlan {
        // 1. 构建邻接表和入度表
        const adjList = new Map<string, string[]>();
        const inDegree = new Map<string, number>();
        
        // 2. Kahn 算法拓扑排序
        const layers: string[][] = [];
        const queue: string[] = [];
        
        // 将入度为 0 的节点入队
        for (const [nodeId, degree] of inDegree.entries()) {
            if (degree === 0) queue.push(nodeId);
        }
        
        while (queue.length > 0) {
            const currentLayer = [...queue];
            queue.length = 0;
            layers.push(currentLayer);
            
            for (const nodeId of currentLayer) {
                for (const neighbor of adjList.get(nodeId) || []) {
                    inDegree.set(neighbor, inDegree.get(neighbor)! - 1);
                    if (inDegree.get(neighbor) === 0) {
                        queue.push(neighbor);
                    }
                }
            }
        }
        
        // 3. 环检测
        if (layers.flat().length !== definition.nodes.length) {
            throw new Error("工作流存在环路");
        }
        
        return { layers, adjacencyList: adjList };
    }
}
```

**时间复杂度**：O(V + E)，V 为节点数，E 为边数。

### 3.2 WorkflowExecutor（事件驱动执行引擎）

**职责**：
- 接收 `WorkflowDefinition` 和 `INodeExecutorAdapter`
- 按拓扑层逐层执行（层内并行）
- 处理条件分支、汇聚节点
- 发射事件供外部订阅

**执行流程**：

```typescript
// applications/orchestrator/src/core/WorkflowExecutor.ts
export class WorkflowExecutor extends EventEmitter {
    public async execute(definition: WorkflowDefinition, globalVars?: Record<string, any>): Promise<WorkflowExecution> {
        const executionId = uuidv4();
        const snapshot = structuredClone(definition); // 深拷贝快照
        const context = new ExecutionContext(globalVars);
        
        // 1. 拓扑排序
        const plan = this._dagParser.parse(snapshot);
        
        // 2. 初始化持久化记录
        await this._persistence.saveExecution({
            executionId,
            workflowId: definition.id,
            status: "running",
            nodeStates: {},
            startedAt: Date.now(),
            snapshot
        });
        
        // 3. 逐层执行
        for (const layer of plan.layers) {
            // 层内并行执行（Promise.allSettled）
            const promises = layer.map(nodeId => this._executeNode(nodeId, snapshot, context, executionId));
            await Promise.allSettled(promises);
        }
        
        // 4. 标记完成
        await this._persistence.updateExecutionStatus(executionId, "completed");
        this.emit("workflowCompleted", executionId);
        
        return await this._persistence.loadExecution(executionId);
    }
    
    private async _executeNode(nodeId: string, snapshot: WorkflowDefinition, context: ExecutionContext, executionId: string): Promise<void> {
        const node = snapshot.nodes.find(n => n.id === nodeId)!;
        
        this.emit("nodeStarted", nodeId, Date.now());
        
        try {
            // 应用重试/超时策略
            const result = await this._nodeStrategy.executeWithStrategy(node, context, this._adapter);
            
            // 保存结果到上下文
            context.setNodeResult(nodeId, result);
            
            // 持久化
            await this._persistence.saveNodeState(executionId, nodeId, { status: "success", ...result });
            
            this.emit("nodeCompleted", nodeId, result);
        } catch (error) {
            await this._persistence.saveNodeState(executionId, nodeId, { status: "failed", error: error.message });
            this.emit("nodeFailed", nodeId, error);
            
            // 如果未开启 skipOnFailure，则终止流程
            if (!node.data.skipOnFailure) {
                throw error;
            }
        }
    }
}
```

### 3.3 ExecutionContext（执行上下文）

**职责**：
- 存储每个节点的执行结果（输出）
- 提供 `getUpstreamOutput(nodeId)` 方法供下游节点访问
- 存储全局变量（如 `startTimeStamp`、`endTimeStamp`）

**数据结构**：

```typescript
// applications/orchestrator/src/core/ExecutionContext.ts
export class ExecutionContext {
    private _nodeResults: Map<string, NodeExecutionResult> = new Map();
    private _globalVars: Map<string, any> = new Map();
    
    public setNodeResult(nodeId: string, result: NodeExecutionResult): void {
        this._nodeResults.set(nodeId, result);
    }
    
    public getUpstreamOutput(nodeId: string): any {
        const result = this._nodeResults.get(nodeId);
        if (!result) {
            throw new Error(`节点 ${nodeId} 的输出不存在（可能尚未执行）`);
        }
        return result.output;
    }
    
    public getGlobalVar(key: string): any {
        return this._globalVars.get(key);
    }
}
```

**防读取过时数据**：引擎按拓扑顺序执行，保证读取时上游节点已完成。

### 3.4 ConditionEvaluator（条件分支逻辑）

**职责**：
- 根据条件表达式求值，返回 `true` / `false`
- 支持 4 种条件类型

**实现**：

```typescript
// applications/orchestrator/src/core/ConditionEvaluator.ts
export class ConditionEvaluator {
    public evaluate(expression: ConditionExpression, context: ExecutionContext): boolean {
        switch (expression.type) {
            case "previousNodeSuccess":
                const prevNodeId = expression.nodeId;
                const prevResult = context.getNodeResult(prevNodeId);
                return prevResult.status === "success";
            
            case "previousNodeFailed":
                return context.getNodeResult(expression.nodeId).status === "failed";
            
            case "keyValueMatch":
                const output = context.getUpstreamOutput(expression.nodeId);
                return output[expression.key] === expression.value;
            
            case "customExpression":
                // 使用 vm2 或类似沙箱执行用户自定义 JS
                return this._evalCustomExpression(expression.code, context);
        }
    }
}
```

### 3.5 NodeExecutionStrategy（重试/超时策略）

**职责**：
- 封装节点执行的重试、超时、跳过逻辑
- 调用 `common/util/retryAsync.ts` 实现重试

**关键代码**：

```typescript
// applications/orchestrator/src/core/NodeExecutionStrategy.ts
export class NodeExecutionStrategy {
    public async executeWithStrategy(node: WorkflowNode, context: ExecutionContext, adapter: INodeExecutorAdapter): Promise<NodeExecutionResult> {
        const { retryCount = 0, timeoutMs = 0, skipOnFailure = false } = node.data;
        
        const executeTask = async () => {
            const promise = adapter.executeTaskNode(node.id, node.data.taskType!, node.data.params || {}, context);
            
            if (timeoutMs > 0) {
                return Promise.race([
                    promise,
                    this._timeout(timeoutMs)
                ]);
            }
            
            return promise;
        };
        
        try {
            return await retryAsync(executeTask, retryCount, 1000);
        } catch (error) {
            if (skipOnFailure) {
                return { success: false, status: "skipped", error: error.message };
            }
            throw error;
        }
    }
    
    private _timeout(ms: number): Promise<never> {
        return new Promise((_, reject) => setTimeout(() => reject(new Error("执行超时")), ms));
    }
}
```

### 3.6 TaskParamsResolver（任务参数解析）

**职责**：
- 合并节点配置参数、执行上下文参数、全局默认参数
- 为每个任务类型生成完整的参数对象
- 实现参数优先级机制

**参数优先级**（从高到低）：
1. **节点配置参数**：在前端属性面板中为节点直接配置的参数
2. **执行上下文参数**：工作流执行时动态注入的参数（如上游节点输出）
3. **全局默认参数**：在 `synthos_config.json` 的 `orchestrator` 配置中定义

**实现示例**：

```typescript
// applications/orchestrator/src/core/TaskParamsResolver.ts
@injectable()
export class TaskParamsResolver {
    public constructor(
        @inject(COMMON_TOKENS.ConfigManagerService) private configManagerService: ConfigManagerService
    ) {}
    
    public async resolveParams<T extends TaskHandlerTypes>(
        taskType: T,
        nodeParams: Record<string, any>,
        context: ExecutionContext
    ): Promise<TaskParamsMap[T]> {
        // 1. 生成默认参数
        const defaultParams = await this._generateDefaultParams(taskType, context);
        
        // 2. 合并节点参数（节点参数优先级更高）
        const mergedParams = { ...defaultParams, ...nodeParams };
        
        return mergedParams as TaskParamsMap[T];
    }
    
    private async _generateDefaultParams(
        taskType: TaskHandlerTypes,
        context: ExecutionContext
    ): Promise<Record<string, any>> {
        const config = await this.configManagerService.getCurrentConfig();
        
        // 从上下文或配置生成时间范围
        let startTimeStamp = context.getGlobalVar("startTimeStamp") as number | undefined;
        let endTimeStamp = context.getGlobalVar("endTimeStamp") as number | undefined;
        
        if (!startTimeStamp || !endTimeStamp) {
            const hoursToCheck = config.orchestrator.defaultTimeRangeInHours || 24;
            endTimeStamp = Date.now();
            startTimeStamp = endTimeStamp - hoursToCheck * 60 * 60 * 1000;
            
            // 保存到上下文供后续节点使用
            context.setGlobalVar("startTimeStamp", startTimeStamp);
            context.setGlobalVar("endTimeStamp", endTimeStamp);
        }
        
        // 从上下文或配置生成 groupIds
        let groupIds = context.getGlobalVar("groupIds") as string[] | undefined;
        if (!groupIds) {
            groupIds = config.orchestrator.defaultGroupIds || [];
            context.setGlobalVar("groupIds", groupIds);
        }
        
        // 根据任务类型生成默认参数
        switch (taskType) {
            case TaskHandlerTypes.ProvideData:
                return {
                    IMType: config.orchestrator.defaultIMType || IMTypes.QQ,
                    groupIds,
                    startTimeStamp,
                    endTimeStamp
                };
            case TaskHandlerTypes.AISummarize:
                return { groupIds, startTimeStamp, endTimeStamp };
            // ... 其他任务类型
        }
    }
}
```

**配置文件示例**：

```json
{
  "orchestrator": {
    "defaultTimeRangeInHours": 100,
    "defaultGroupIds": ["12345678", "87654321"],
    "defaultIMType": "QQ"
  }
}
```

**前端参数配置界面**：
- **任务节点属性面板**：根据任务类型动态展示参数表单（groupIds、startTimeStamp、endTimeStamp、IMType、reportType 等）
- **全局参数设置按钮**：在工作流页面工具栏，可编辑 `defaultTimeRangeInHours`、`defaultGroupIds`、`defaultIMType`

### 3.7 ExecutionPersistence（SQLite 持久化）

**职责**：
- 创建 SQLite 数据库 `synthos_workflow_executions.db`
- 存储 `WorkflowExecution` 表
- 提供 `saveExecution`、`loadExecution`、`listExecutions` 方法

**Schema**：

```sql
CREATE TABLE workflow_executions (
    execution_id TEXT PRIMARY KEY,
    workflow_id TEXT NOT NULL,
    status TEXT NOT NULL,  -- "pending" | "running" | "completed" | "failed"
    node_states TEXT,      -- JSON
    started_at INTEGER,
    completed_at INTEGER,
    snapshot TEXT          -- JSON（工作流定义快照）
);
```

**断点续跑实现**：

```typescript
public async resumeExecution(executionId: string): Promise<WorkflowExecution> {
    const execution = await this.loadExecution(executionId);
    if (!execution) {
        throw new Error("执行记录不存在");
    }
    
    // 跳过已完成的节点
    const completedNodes = Object.keys(execution.nodeStates).filter(
        nodeId => execution.nodeStates[nodeId].status === "success"
    );
    
    // 重新执行失败/取消的节点
    const restartNodes = Object.keys(execution.nodeStates).filter(
        nodeId => ["failed", "cancelled"].includes(execution.nodeStates[nodeId].status)
    );
    
    // 调用 WorkflowExecutor 继续执行
    return this._executor.resume(execution.snapshot, completedNodes, restartNodes);
}
```

---

## 4. 数据流与执行模型

### 4.1 端到端数据流

```
1. 用户在前端拖拽节点 → 生成 WorkflowDefinition JSON
2. 点击"保存" → POST /api/workflow/save → 写入 synthos_config.json
3. 点击"手动触发" → POST /api/workflow/trigger → tRPC Client
4. tRPC Client → Orchestrator tRPC Server → WorkflowExecutor.execute()
5. Executor 解析 DAG → 逐层执行节点
6. 每个节点执行时 → AgendaNodeExecutorAdapter.executeTaskNode()
7. Adapter 调用 agendaInstance.now() → 任务进入 Agenda 队列
8. Adapter 轮询 MongoDB (Agenda jobs) 等待任务完成
9. 任务完成后 → 更新 ExecutionContext + 持久化到 SQLite
10. Executor 发射 nodeCompleted 事件 → tRPC Server 推送给前端
11. 前端 WebSocket 收到事件 → 更新 Zustand store → React Flow 节点重新渲染
```

### 4.2 并行执行模型

Kahn 算法生成的 `ExecutionPlan` 是一个**分层列表**：

```typescript
{
  layers: [
    ["start"],                    // 第 0 层：起始节点
    ["provide-data"],             // 第 1 层
    ["preprocess"],               // 第 2 层
    ["ai-summarize"],             // 第 3 层
    ["generate-embedding", "interest-score"],  // 第 4 层：并行
    ["end"]                       // 第 5 层
  ]
}
```

**关键点**：
- 每层内的节点 **完全独立**，可以并行执行
- 使用 `Promise.allSettled()` 并发调用，避免一个节点失败导致其他节点被取消
- 层与层之间严格串行，保证依赖关系

### 4.3 条件分支执行模型

条件节点有多条出边，执行时：

1. 求值条件表达式 → 得到 `true` / `false`
2. 根据结果选择对应的出边
3. **只执行选中分支** 的下游节点
4. 未选中分支的下游节点状态标记为 `skipped`

**示例**：

```
[ProvideData] → [Condition]
                  ├── true → [Preprocess]
                  └── false → [AISummarize]
```

如果条件求值为 `false`，则 `Preprocess` 节点被跳过，直接执行 `AISummarize`。

---

## 5. 持久化方案

### 5.1 为什么选择 SQLite

| 对比项 | SQLite | MongoDB |
|--------|--------|---------|
| **部署复杂度** | ✅ 单文件，无需独立服务 | ❌ 需要 MongoDB 服务 |
| **ACID 事务** | ✅ 支持 | ✅ 支持 |
| **查询灵活性** | ✅ 标准 SQL | ⚠️ BSON 查询语法 |
| **备份** | ✅ 直接复制文件 | ❌ 需要 mongodump |
| **适用场景** | ✅ 本地持久化、历史回溯 | ✅ 分布式、高并发 |

考虑到工作流执行记录通常在**单机环境**下查询（不需要分布式），且数据量不大（每次执行一条记录），SQLite 更轻量。

### 5.2 数据库Schema设计

#### 表：workflow_executions

| 字段 | 类型 | 说明 |
|------|------|------|
| `execution_id` | TEXT (PK) | UUID，唯一标识每次执行 |
| `workflow_id` | TEXT | 工作流 ID（来自 synthos_config.json） |
| `status` | TEXT | "pending" / "running" / "completed" / "failed" |
| `node_states` | TEXT (JSON) | 各节点状态：`{ [nodeId]: { status, startedAt, completedAt, output, error } }` |
| `started_at` | INTEGER | 开始时间（UNIX 毫秒时间戳） |
| `completed_at` | INTEGER | 完成时间 |
| `snapshot` | TEXT (JSON) | 工作流定义快照（深拷贝） |

#### 索引

```sql
CREATE INDEX idx_workflow_id ON workflow_executions(workflow_id);
CREATE INDEX idx_status ON workflow_executions(status);
CREATE INDEX idx_started_at ON workflow_executions(started_at DESC);
```

### 5.3 运行时快照机制

**问题**：如果用户在工作流运行期间修改了工作流定义，会导致执行状态与定义不一致。

**解决方案**：执行器启动时，立即 `structuredClone(workflowDefinition)` 创建快照，保存到 SQLite 的 `snapshot` 字段。执行期间完全基于快照运行，不受外部修改影响。

---

## 6. tRPC 通信链路

### 6.1 架构分层

```
Frontend (React)
    ↓ tRPC Client (WebSocket)
WebUI-Backend (:3002/trpc)
    ↓ tRPC Client (转发)
Orchestrator (:8081)
    ↓ tRPC Server
```

**为什么要分两层？**
1. **安全隔离**：前端不直接访问 Orchestrator，所有请求经过 webui-backend 统一鉴权
2. **协议转换**：webui-backend 可以将 REST 请求转换为 tRPC 调用
3. **统一网关**：未来可以在 webui-backend 层实现限流、日志、监控等中间件

### 6.2 tRPC Router 定义

```typescript
// common/rpc/orchestrator/router.ts
export function createOrchestratorRouter(impl: OrchestratorRPCImplementation) {
    return t.router({
        // Query: 查询类接口
        listWorkflows: t.procedure
            .output(ListWorkflowsOutputSchema)
            .query(async () => impl.listWorkflows()),
        
        getWorkflow: t.procedure
            .input(GetWorkflowInputSchema)
            .output(GetWorkflowOutputSchema)
            .query(async ({ input }) => impl.getWorkflow(input)),
        
        // Mutation: 变更类接口
        triggerWorkflow: t.procedure
            .input(TriggerWorkflowInputSchema)
            .output(TriggerWorkflowOutputSchema)
            .mutation(async ({ input }) => impl.triggerWorkflow(input)),
        
        // Subscription: 实时推送
        onExecutionUpdate: t.procedure
            .input(OnExecutionUpdateInputSchema)
            .subscription(async ({ input }) => {
                return observable<ExecutionUpdateEvent>((emit) => {
                    const listener = (event: ExecutionUpdateEvent) => {
                        if (event.executionId === input.executionId) {
                            emit.next(event);
                        }
                    };
                    
                    impl.subscribeToExecutionUpdates(listener);
                    
                    return () => {
                        impl.unsubscribeFromExecutionUpdates(listener);
                    };
                });
            })
    });
}
```

### 6.3 前端 Subscription 示例

```typescript
// applications/webui-frontend/src/pages/workflow/hooks/useExecutionStatus.ts
import { trpc } from '@/api/trpcClient';

export function useExecutionStatus(executionId: string) {
    const [nodeStates, setNodeStates] = useState<Record<string, NodeState>>({});
    
    useEffect(() => {
        const subscription = trpc.onExecutionUpdate.subscribe(
            { executionId },
            {
                onData(event) {
                    if (event.type === "nodeCompleted") {
                        setNodeStates(prev => ({
                            ...prev,
                            [event.nodeId]: { status: "success", ...event.result }
                        }));
                    }
                },
                onError(err) {
                    console.error("订阅错误:", err);
                }
            }
        );
        
        return () => subscription.unsubscribe();
    }, [executionId]);
    
    return nodeStates;
}
```

---

## 7. 前端技术选型

### 7.1 React Flow v12

**核心功能**：
- 拖拽节点和连线
- 自动布局（可选）
- 缩放、平移、小地图
- 自定义节点组件

**自定义节点示例**：

```tsx
// applications/webui-frontend/src/pages/workflow/nodes/TaskNode.tsx
import { memo } from 'react';
import { Handle, Position, NodeProps } from '@xyflow/react';

export const TaskNode = memo(({ data }: NodeProps) => {
    const statusColor = {
        pending: 'border-gray-400',
        running: 'border-blue-500 animate-pulse',
        success: 'border-green-500',
        failed: 'border-red-500'
    }[data.status || 'pending'];
    
    return (
        <div className={`px-4 py-2 rounded border-2 ${statusColor} bg-white`}>
            <Handle type="target" position={Position.Left} />
            <div className="text-sm font-medium">{data.label}</div>
            <div className="text-xs text-gray-500">{data.taskType}</div>
            <Handle type="source" position={Position.Right} />
        </div>
    );
});
```

### 7.2 Zustand 状态管理

**为什么不用 Redux？**
- Redux 需要大量样板代码（actions、reducers、store）
- Zustand 仅 3KB，API 极简
- React Flow 官方推荐

**Store 示例**：

```typescript
// applications/webui-frontend/src/pages/workflow/stores/workflowStore.ts
import { create } from 'zustand';

interface WorkflowStore {
    nodes: Node[];
    edges: Edge[];
    selectedNodeId: string | null;
    setNodes: (nodes: Node[]) => void;
    setEdges: (edges: Edge[]) => void;
    updateNodeData: (nodeId: string, data: Partial<NodeData>) => void;
}

export const useWorkflowStore = create<WorkflowStore>((set) => ({
    nodes: [],
    edges: [],
    selectedNodeId: null,
    
    setNodes: (nodes) => set({ nodes }),
    setEdges: (edges) => set({ edges }),
    
    updateNodeData: (nodeId, data) => set((state) => ({
        nodes: state.nodes.map(node =>
            node.id === nodeId ? { ...node, data: { ...node.data, ...data } } : node
        )
    }))
}));
```

### 7.3 Monaco Editor 集成（脚本节点）

脚本节点需要内嵌代码编辑器，复用项目中已有的 `@monaco-editor/react`：

```tsx
// applications/webui-frontend/src/pages/workflow/components/property-forms/ScriptPropertyForm.tsx
import Editor from '@monaco-editor/react';

export function ScriptPropertyForm({ node, onChange }) {
    return (
        <div className="h-64">
            <Editor
                height="100%"
                language="javascript"
                theme="vs-dark"
                value={node.data.scriptCode || ''}
                onChange={(value) => onChange({ scriptCode: value })}
                options={{
                    minimap: { enabled: false },
                    lineNumbers: 'on',
                    tabSize: 2
                }}
            />
        </div>
    );
}
```

---

## 8. 性能与优化

### 8.1 并发控制

**问题**：默认 Agenda `maxConcurrency: 1`，所有任务串行执行，无法利用 DAG 的并行能力。

**解决方案**：
1. 调高 Agenda 的 `maxConcurrency` 配置（如设为 5）
2. 在 `AgendaNodeExecutorAdapter` 中实现并发控制：

```typescript
class AgendaNodeExecutorAdapter {
    private _concurrencySemaphore = new Semaphore(5);
    
    async executeTaskNode(nodeId, taskType, params, context) {
        await this._concurrencySemaphore.acquire();
        try {
            const job = await agendaInstance.now(taskType, params);
            return await this._waitForJobCompletion(job.attrs._id);
        } finally {
            this._concurrencySemaphore.release();
        }
    }
}
```

### 8.2 轮询优化

**问题**：`waitForJobCompletion` 轮询 MongoDB 查询任务状态，间隔 1 秒，可能浪费资源。

**优化方案**：
1. **短期**：使用指数退避（初始 500ms，最大 5s）
2. **长期**：改用 MongoDB Change Streams 实时监听 jobs 集合变化

### 8.3 前端渲染优化

**问题**：React Flow 渲染大量节点时（> 100）可能卡顿。

**优化方案**：
1. 使用 `memo()` 包裹节点组件，避免不必要的重渲染
2. 启用 React Flow 的 `nodesDraggable={false}` 在执行时禁用拖拽
3. 使用虚拟化（`react-virtualized`）渲染执行历史列表

---

## 9. 测试策略

### 9.1 单元测试（Vitest）

**覆盖目标**：
- `DagParser`：拓扑排序正确性、环检测
- `ConditionEvaluator`：各种条件类型
- `NodeExecutionStrategy`：重试、超时、跳过
- `ExecutionContext`：节点输出存储与读取

**测试用例示例**：

```typescript
// applications/orchestrator/src/test/DagParser.test.ts
import { describe, it, expect } from 'vitest';
import { DagParser } from '../core/DagParser';

describe('DagParser', () => {
    it('应正确识别环路', () => {
        const definition = {
            nodes: [
                { id: 'A', type: 'task', position: { x: 0, y: 0 }, data: {} },
                { id: 'B', type: 'task', position: { x: 0, y: 0 }, data: {} },
                { id: 'C', type: 'task', position: { x: 0, y: 0 }, data: {} }
            ],
            edges: [
                { id: 'e1', source: 'A', target: 'B' },
                { id: 'e2', source: 'B', target: 'C' },
                { id: 'e3', source: 'C', target: 'A' }  // 环路
            ]
        };
        
        const parser = new DagParser();
        expect(() => parser.parse(definition)).toThrow('工作流存在环路');
    });
    
    it('应正确生成分层执行计划', () => {
        const definition = {
            nodes: [
                { id: 'start', type: 'start', position: { x: 0, y: 0 }, data: {} },
                { id: 'A', type: 'task', position: { x: 0, y: 0 }, data: {} },
                { id: 'B', type: 'task', position: { x: 0, y: 0 }, data: {} },
                { id: 'C', type: 'task', position: { x: 0, y: 0 }, data: {} }
            ],
            edges: [
                { id: 'e1', source: 'start', target: 'A' },
                { id: 'e2', source: 'A', target: 'B' },
                { id: 'e3', source: 'A', target: 'C' }  // B 和 C 并行
            ]
        };
        
        const parser = new DagParser();
        const plan = parser.parse(definition);
        
        expect(plan.layers).toEqual([
            ['start'],
            ['A'],
            ['B', 'C']  // 第 3 层：并行
        ]);
    });
});
```

### 9.2 集成测试

**测试场景**：
- 端到端工作流执行（使用 `MockNodeExecutorAdapter`）
- 断点续跑（模拟失败场景）
- tRPC subscription 推送（使用 WebSocket 测试客户端）

### 9.3 测试覆盖率

**目标**：> 80%

当前覆盖情况（截至 2026-02-07）：
- `DagParser`: 95%
- `WorkflowExecutor`: 88%
- `ExecutionContext`: 100%
- `ConditionEvaluator`: 90%
- `NodeExecutionStrategy`: 85%

---

## 10. 扩展性设计

### 10.1 自定义节点类型

**场景**：未来需要支持数据库查询节点、邮件发送节点等。

**扩展方式**：
1. 在 `WorkflowNodeType` 枚举中新增类型：`Database = "database"`
2. 在 `INodeExecutorAdapter` 接口中新增方法：`executeDatabaseNode()`
3. 在 `WorkflowExecutor` 中添加对应的 `case` 分支
4. 前端新增节点组件和属性表单

**示例**：

```typescript
// common/contracts/workflow/index.ts
export enum WorkflowNodeType {
    // ...
    Database = "database"
}

// applications/orchestrator/src/adapters/INodeExecutorAdapter.ts
export interface INodeExecutorAdapter {
    // ...
    executeDatabaseNode(nodeId: string, query: string, context: ExecutionContext): Promise<NodeExecutionResult>;
}
```

### 10.2 插件化节点执行器

**目标**：允许第三方开发者编写自定义节点执行器（如调用外部 API）。

**设计**：
1. 定义 `NodeExecutorPlugin` 接口
2. 在配置文件中注册插件路径
3. Orchestrator 启动时动态加载插件

```typescript
export interface NodeExecutorPlugin {
    name: string;
    supportedNodeTypes: WorkflowNodeType[];
    execute(node: WorkflowNode, context: ExecutionContext): Promise<NodeExecutionResult>;
}
```

### 10.3 子工作流（Sub-Workflow）

**场景**：一个工作流中嵌套另一个工作流（如批量处理）。

**设计要点**：
- 新增 `SubWorkflow` 节点类型
- 节点配置中指定子工作流 ID
- 执行时递归调用 `WorkflowExecutor.execute()`

---

## 11. 已知问题与未来规划

### 11.1 已知问题

| 问题 | 影响 | 优先级 | 状态 |
|------|------|--------|------|
| Agenda `maxConcurrency=1` 限制并行 | 中 | P1 | 计划中 |
| 轮询 MongoDB 性能开销 | 低 | P2 | 计划中 |
| 脚本节点缺少沙箱隔离 | 高（安全风险） | P0 | **待修复** |
| 无法动态修改运行中的工作流 | 低 | P3 | 已知限制 |
| 前端渲染大量节点时卡顿 | 中 | P2 | 已有优化方案 |

### 11.2 未来规划

#### Phase 3（计划中）：前端可视化增强
- ✅ React Flow 基础集成（已完成）
- ✅ 自定义节点组件（已完成）
- ✅ 拖拽添加节点（已完成）
- ⏳ 自动布局（Dagre 算法）
- ⏳ 节点搜索与过滤
- ⏳ 工作流模板市场

#### Phase 4（已暂停）：日志系统改造
- Logger 增加 `executionId` 标签
- 后端 Logs API 支持按 `executionId` 过滤
- 前端内嵌日志面板

**暂停原因**：当前日志可通过系统监控页面查看，优先级降低。

#### Phase 5（长期）：分布式执行
- **问题**：单机 Orchestrator 无法水平扩展
- **方案**：
  - 引入分布式锁（Redis）
  - 多个 Orchestrator 实例竞争执行权
  - 使用 MongoDB Change Streams 实时同步状态

#### Phase 6（探索中）：AI 辅助编排
- **功能**：用户输入自然语言需求，AI 自动生成工作流
- **技术栈**：LangChain + 自定义 Prompt
- **示例**：
  - 输入："每天早上 9 点拉取昨天的聊天记录，生成摘要并发送邮件"
  - 输出：一个完整的 WorkflowDefinition JSON

---

## 12. 参考资料

### 12.1 核心依赖文档
- [React Flow Documentation](https://reactflow.dev/)
- [tRPC Documentation](https://trpc.io/)
- [Zustand Documentation](https://zustand-demo.pmnd.rs/)
- [Agenda Documentation](https://github.com/agenda/agenda)

### 12.2 算法与设计模式
- [Kahn 拓扑排序算法](https://en.wikipedia.org/wiki/Topological_sorting)
- [适配器模式（Adapter Pattern）](https://refactoring.guru/design-patterns/adapter)
- [观察者模式（Observer Pattern / EventEmitter）](https://nodejs.org/api/events.html)

### 12.3 内部文档
- [Synthos 主文档](../../README.md)
- [工作流编排使用指南](./工作流编排使用指南.md)（用户手册）
- [Orchestrator 模块文档](../../applications/orchestrator/README.md)
- [API 文档 - Orchestrator RPC 接口](./接口文档/API文档.md#21-orchestrator-rpc-接口)

---

## 13. 任务注册机制重构（装饰器模式）

### 13.1 重构背景与问题

在工作流系统初期版本中，任务定义采用**硬编码枚举**方式，违反开闭原则：

- **开闭原则违反**：新增任务需修改 5+ 文件（enum、interface、resolver、前端组件）
- **缺乏运行时校验**：参数类型仅编译时检查
- **前后端不同步**：前端任务列表手动维护
- **UI 表单硬编码**：针对每个任务手写 switch-case
- **元数据分散**：显示名称、描述、参数逻辑分散多处

### 13.2 重构方案：装饰器 + TaskRegistry

采用**装饰器模式**实现任务自动注册和元数据管理，核心架构：

```typescript
// 1. 定义 Zod Schema
const ParamsSchema = z.object({
    groupIds: z.array(z.string()).min(1),
    startTimeStamp: z.number().int().positive()
});

// 2. 使用 @Task 装饰器注册
@injectable()
@Task({
    displayName: "AI 摘要生成",
    paramsSchema: ParamsSchema,
    generateDefaultParams: async (context, config) => ({...}),
    uiConfig: {
        icon: "🤖",
        formFields: [
            { name: "groupIds", type: "array", label: "群组列表", required: true },
            { name: "startTimeStamp", type: "timestamp", label: "开始时间" }
        ]
    }
})
export class AISummarizeTaskHandler {
    public static readonly TASK_NAME = "AISummarize";
}
```

**核心组件**：

1. **TaskRegistry**：单例，管理所有任务元数据（`Map<string, TaskMetadata>`）
2. **@Task 装饰器**：从类的静态属性获取 `TASK_NAME`，自动调用 `registry.register()`
3. **Zod Schema**：运行时参数校验，可转换为 JSON Schema 供前端使用
4. **FormFieldConfig**：UI 表单字段配置，驱动前端动态渲染

### 13.3 前后端集成

**后端**：
- TaskParamsResolver 优先从 TaskRegistry 获取默认参数生成函数和校验逻辑
- `/api/tasks/registry` 接口暴露所有任务元数据（含 JSON Schema）
- 保留 switch-case 作为回退（向后兼容）

**前端**：
- 从 `/api/tasks/registry` 动态加载任务列表
- `DynamicParamsForm` 组件根据 `formFields` 动态渲染表单
- TaskPropertyForm 不再硬编码 TASK_TYPES，改为从 API 获取

### 13.4 重构效果对比

| 维度 | 重构前 | 重构后 |
|------|--------|--------|
| 新增任务需修改文件数 | 5+ 个 | 1 个（仅任务处理器） |
| 参数校验 | 仅编译时 | 编译时 + 运行时（Zod） |
| 前后端同步 | 手动同步 | 自动同步（API 驱动） |
| UI 表单 | 硬编码 switch-case | 元数据驱动，动态渲染 |
| 开闭原则 | ❌ 违反 | ✅ 符合 |

### 13.5 添加新任务步骤（重构后）

1. 创建任务处理器类，定义 Zod Schema
2. 添加 `@Task` 装饰器，配置 `displayName`、`uiConfig` 等
3. 实现 `register()` 方法注册到 Agenda
4. **完成！**前端自动获取并显示新任务

---

**撰写日期**：2026-02-07  
**作者**：Synthos 开发团队  
**版权**：© 2026 Synthos Project. All Rights Reserved.
