...
引擎不依赖具体任务实现，通过 `NodeExecutorAdapter` 接口解耦：
...
**职责**：
- 接收 `WorkflowDefinition` 和 `NodeExecutorAdapter`
- 按拓扑层逐层执行（层内并行）
- 处理条件分支、汇聚节点
- 发射事件供外部订阅
...
### 3.6 ExecutionPersistence（SQLite 持久化）
...
2. 在 `NodeExecutorAdapter` 接口中新增方法：`executeDatabaseNode()`
